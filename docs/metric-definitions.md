...
| 指标 | 当前口径 |
| --- | --- |
| `firstChaseStartTime` | `trial_start` 到第一条游戏正式 `chase_start` 的时间。该指标表示多久首次建立正式追逐压力，不等同于玩家主观发现目标的准确时刻。 |
...
`averageChaseGap` 仅作为详细说明数据，不单独形成主要节奏断点，因为空窗可能同时包含搬运、挂钩、控机、巡逻和转场，系统不据此猜测玩家意图。
